...
Cómo ejecutar
--------------
- Toda la suite en modo silencioso:  
  `pytest -q`
- Toda la suite mostrando prints (`-s`):  
  `pytest -s`
- Solo los unit tests:  
  `pytest tests/unit -q`
- Solo las pruebas de integración:  
  `pytest tests/integration -q`
- Archivo específico con prints (útil para depurar):  
  `pytest tests/unit/test_model.py -s`
- Cobertura con reporte HTML guardado en `tests/coverage_html`:  
...
- `tests/integration/test_pipeline_dvc.py`: ejecuta el pipeline completo mediante `dvc repro` y valida que todas las stages (`data`, `train`, `evaluate`, `visualize`) se ejecuten sin errores. Comprueba la existencia de artefactos clave como:
  - Datos procesados en `data/processed/` (train/test limpios).
  - Modelos entrenados en `models/` (archivos `.pkl`).
  - Métricas de evaluación en `metrics/` (JSON por modelo).
  - Reportes finales en `reports/` (gráficas o resúmenes de performance).
- Estas pruebas verifican la integración entre el código Python, la configuración declarativa de `dvc.yaml` y la estructura de directorios del proyecto, asegurando que un usuario pueda reproducir el flujo end-to-end a partir de los comandos estándar de DVC y pytest.
